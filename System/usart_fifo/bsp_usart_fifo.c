...
/*
*   函 数 名: InitHardUart
*   功能说明: 配置串口的硬件参数（波特率，数据位，
*   停止位，起始位，校验位，中断使能）适合于STM32-H7开发板
*   形    参: 无
*   返 回 值: 无
*   时    间: 2024年7月7日 15:48:05
*/
void InitHardUart(void)
{
    ...
    /* 配置波特率、奇偶校验 */
    bsp_SetUartParam(USART1,  UART1_BAUD, UART_PARITY_NONE,\
                        UART_MODE_TX_RX);
    ...
    /* 配置波特率、奇偶校验 */
    bsp_SetUartParam(USART2,  UART2_BAUD, UART_PARITY_NONE,\
                        UART_MODE_TX_RX);
    ...
    /* 配置波特率、奇偶校验 */
    bsp_SetUartParam(USART3,  UART3_BAUD, UART_PARITY_NONE,\
                        UART_MODE_TX_RX);
...
*              USART3, UART4, UART5，USART6，UART7，UART8
*   时    间: 2024年7月7日 15:54:57
*/
...
*   形    参: 无
*   返 回 值: 无
*   时    间: 2024年7月7日 16:11:10
*/
static void UartSend(UART_T *_pUart,uint8_t *_ucaBuf,
                            uint16_t _usLen)
{
    uint16_t i;
...
/*
*   函 数 名: comSendBuf
*   功能说明: 向串口发送一组数据。数据放到发送缓冲区后立即返回
*             由中断服务程序在后台完成发送
*   形    参: _ucPort: 端口号(COM1 - COM8)
*             _ucaBuf: 待发送的数据缓冲区
*             _usLen : 数据长度
*   返 回 值: 无
*   时    间: 2024年7月7日 16:13:59
*/
void comSendBuf(COM_PORT_E _ucPort,uint8_t *_ucaBuf,
                    uint16_t _usLen)
{
    UART_T *pUart;
    ...
    {
        /* 如果是RS485通信，可以在这个函数中将RS485设置为发送模 */
        pUart->SendBefor();
    }
...
*   函 数 名: fgetc
*   功能说明: 重定义getc函数
*               这样可以使用getchar函数从串口1输入数据
*   形    参: 无
*   返 回 值: 无
*   时    间: 2024年7月7日 16:21:07
*/
#if DEBUG_SWITCH_EN == 0
int fgetc(FILE *f)
{
